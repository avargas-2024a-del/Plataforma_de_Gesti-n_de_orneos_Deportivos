...
import { jugadoresService, equiposService, CreateJugadorDto, Equipo } from "@/services";

export default function NewJugadorPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [equipos, setEquipos] = useState<Equipo[]>([]);
  const [form, setForm] = useState<CreateJugadorDto>({
    nombres: "",
    apellidos: "",
    dorsal: 0,
    posicion: "",
  ...
  useEffect(() => {
    equiposService.findAll().then(setEquipos);
  }, []);
